...
{
    float distance = distanceBetweenToPoints(cs.center, playerpos);
    if(!chasing && (distance<=50.0) && (distance>=1.0))
    {
        chasing=true;
        start=SDL_GetTicks();
    }
    if(chasing && (SDL_GetTicks()-start)<=7000)
    {
        if(!isdead && (distance<=50.0) && (distance>=1.0))
        {
            direction.change(playerpos-cs.center);
            direction.normalize();
...
//	std::cout << isdead << iswalk << isattack << std::endl;
    ...
    if(isdead && curframe==walk+attack+die-1)
    {
        hasdied=true;
        //del();
    }
...
float zombie::distanceBetweenToPoints(vector3d enemyloc,vector3d playerloc)
{
    return sqrt((playerloc.x-enemyloc.x) * (playerloc.x-enemyloc.x) + (playerloc.y-enemyloc.y) * (playerloc.y-enemyloc.y) + (playerloc.z-enemyloc.z) * (playerloc.z-enemyloc.z));
}

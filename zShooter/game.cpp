...
game::game()
{
    int screenX = DEFAULT_SCREEN_X, screenY = DEFAULT_SCREEN_Y;
    ...
    std::cout << "Loading-- (this may take a while!) ...\n\n";
        ...
        if(inputc == 'y' || inputc == 'Y')
            screen=SDL_SetVideoMode(screenX,screenY,32,SDL_SWSURFACE|SDL_OPENGL|SDL_FULLSCREEN);
        else
            screen=SDL_SetVideoMode(screenX,screenY,32,SDL_SWSURFACE|SDL_OPENGL);
        SDL_WM_SetCaption("zShooter v0.95",NULL);
    #else
        screen=SDL_SetVideoMode(screenX,screenY,32,SDL_SWSURFACE|SDL_OPENGL);
        SDL_WM_SetCaption("zShooter v0.95 - Test Build",NULL);
    #endif // _TEST_GAME_
    ...
    glLoadIdentity();
    gluPerspective(45,(float)screenX/(float)screenY,0.1,500.0);
...
void game::update()
{
    ...
    if(isfired)
    {
        ...
        if(isshot)
        {
            ...
            {
                if(collision::raysphere(zombies[i].getSphere()->center.x,zombies[i].getSphere()->center.y,zombies[i].getSphere()->center.z,direction.x,direction.y,direction.z,player1->cam.getLocation().x,player1->cam.getLocation().y,player1->cam.getLocation().z,0.45))
                {
...
void game::drawMenu(int health,int ammo,int allammo,int point,std::string& weaponName)
{
    ...
    #ifdef _TEST_GAME_
        sprintf(tmp,"WEAPON: %s  %d / %d [%d]",weaponName.c_str(),ammo,allammo,player1->getCurWeapon()->getStrength());
    #else
        sprintf(tmp,"WEAPON: %s    %d / %d",weaponName.c_str(),ammo,allammo);
    #endif // _TEST_GAME_
